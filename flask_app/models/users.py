...
class User:
    ...
    def add_friendship(cls, data):
        query = """INSERT INTO friendships (user_id, friend_id)	SELECT %(first_user)s, %(second_user)s	
                    WHERE( 
                        SELECT NOT EXISTS( 
                            SELECT user_id, friend_id	
                            FROM friendships 
                            WHERE (user_id = %(first_user)s AND friend_id = %(second_user)s) OR (user_id = %(second_user)s AND friend_id = %(first_user)s)));"""
